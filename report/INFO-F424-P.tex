...
Considering that $n$ and $K \in \N$, the \#variables < \#constraints. For $P_1$ this is obvious. For $P_2$, we have to solve the inequality $n + K > nK + 2$ in order to have an improvement in the number of variables by taking the dual. Using Wolframalpha, the solution of this inequation for n > 1 is $K = \dfrac{n-2}{n-1} < 1$. As K is an integer, the inequation is $K \leq 0$ which does not give any solution as $K > 0$ by definition.
