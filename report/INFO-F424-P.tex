...
As Julia is a scripting language, nothing needs to be compiled. However, one might have to install the following packages to ensure the correction functioning of our program:
\begin{itemize}
	\item \texttt{ArgParse}: used to parse command-line arguments
	\item \texttt{Cbc}: a simple LP solver
	\item \texttt{Gurobi}: a more complex, commercial LP solver
\end{itemize}
...
\begin{itemize}
    \item[-] ``main.jl`` 
    \item[-] ``P1.jl`` : Formulation $P_1$
    \item[-] ``P3.jl`` : Formulation $P_3$
    \item[-] ``run\_all\_gurobi\_P1.sh`` : run all instances with formulation $P_1$ with Gurobi as solver 
    \item[-] ``run\_all\_gurobi\_P3.sh`` : run all instances with formulation $P_3$ with Gurobi as solver 
        \item[-] ``chvatal-gomory-divisor-search.sh`` :  
    \item[-] ``chvatal-gomor-divisor-search-2.sh`` : 
    \item[-] ``README.md`` : Read me containing primary informations on the project and how to compile
    \item[-] ``instances/`` : all the instances to test
    \item[-] ``out/`` : outputs
    \item[-] ``report/`` : report (in \LaTeX \; and in PDF)
    \item[-] ``resources/`` : paper and project formulation
    \end{itemize}
    
\subsection{Helper functions} 

\paragraph{PCInstance.jl} Type PCInstance, contains 5 attributes: n, p, d, K, rho.

\paragraph{read\_instance.jl} Input: path, Output: instance::PCInstance

Parse an instance (given in path) and returns a PCInstance

\paragraph{Result.jl}  Type Result, contains 2 attributes: score, time.

\paragraph{twoapprox\_heuristic.jl} Input: instance::PCInstance, Output: solution::Array{UInt8,n}

Function returning the 2-approximation heuristic of an instance

\paragraph{random\_heuristic.jl} Input: instance::PCInstance, Output: solution::Array{UInt8,n}

Function returning a random solution of an instance

\paragraph{bestHeuristicRandom.jl} Input: instance::PCInstance, Output: solution::Array{UInt8,n}

Returns the best solution out of 1000 random solutions

\paragraph{bestHeuristicTwoApprox.jl} Input: instance::PCInstance, Output: solution::Array{UInt8,n}

Select the best solution out of 10 solutions found by the 2-approx heuristic

\paragraph{bestHeuristicMix.jl} Input: instance::PCInstance, Output: solution::Array{UInt8,n}

Select the best solution out of 10 solutions found by the 2-approximation heuristic and 1000 random solutions.

\paragraph{print\_solution.jl} Input: instance::PCInstance, solution::Array{UInt8,n}

Print the position of the centers.

\paragraph{avg-divisor-search.py}

Python file to realize the grid search on the parameter ``divisor``
...
We tried looking for a value of the divisor which would improve the performance as much as possible. It is clear that the strength of the inequality depends on how the floor operation cuts the numbers. So both the values in the distance matrix and the divisor have an important impact. Let us run our program using Cbc on the first instance to have some insight on how the divisor impacts the execution time:
